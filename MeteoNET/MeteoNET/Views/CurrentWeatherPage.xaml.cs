...
namespace MeteoNET.Views
{
    ...
    {
        private string Language = Preferences.Get("Idioma", "sp");
        private string Units = Preferences.Get("Unidades", "metric");
        ...
        protected override void OnAppearing()
        {
            ...
            // Actualizar los valores de idioma y unidades
            Language = Preferences.Get("Idioma", "sp");
            Units = Preferences.Get("Unidades", "metric");
            GetCoordinates();
        }
        ...
        public void OnSearchButtonPressed(object sender, EventArgs e)
        {
            ...
            databaseHelper.AgregarLocalizacion(localizacion);
            GetWeatherInfo();
        }
        ...
        {
            var url = $"http://api.openweathermap.org/data/2.5/weather?q={_Location}&lang={Language}&appid={API_KEY}&units={Units}";
        ...
        {
            var url = $"http://api.openweathermap.org/data/2.5/forecast?q={Location}&lang=sp&appid={API_KEY}&units=metric";
            var result = await ApiCaller.Get(url);
